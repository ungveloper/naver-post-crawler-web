...
export async function GET(request: Request) {
  ...
  try {
    ...
    const html = buildArticleHtmlFile(
      data.title,
      data.contentHtml,
      data.sourceUrl,
    );
    const fileName = `${safeFilename(data.title) || 'naver-blog-article'}.html`;
    ...
    return new Response(
      error instanceof Error ? error.message : 'HTML 파일 생성에 실패했습니다.',
      { status: 500 },
    );
  }
}
